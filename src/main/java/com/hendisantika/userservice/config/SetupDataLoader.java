...
public class SetupDataLoader implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(final ContextRefreshedEvent event) {
        ...
        Role libRole = createRoleIfNotFound(Role.ROLE_LIBRARIAN);
        createUserIfNotFound("admin@gmail.com", Set.of(adminRole));
        alreadySetup = true;
    }
    ...
    private final User createUserIfNotFound(final String email, Set<Role> roles) {
        ...
        if (user == null) {
            ...
            user.setEmail(email);
            user.setPassword(passwordEncoder.encode("admin1234567"));
            user.setRoles(roles);
